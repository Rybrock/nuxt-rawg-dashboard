    ...
    <div class="min-h-screen bg-gray-900 text-white">
      ...
      <template v-else>
        ...
        <div class="relative lg:h-64 md:h-56 h-48">
          ...
          />
          <div class="absolute bottom-0 left-0 right-0 z-20 p-6 md:p-10 bg-gradient-to-t from-gray-900 to-transparent">
        ...
        <!-- Screenshots thumbnails section -->
        <div v-if="screenshots.length > 0" class="container mx-auto py-3 px-4 md:px-6">
          <div class="p-3 rounded-lg w-auto">
            <div class="flex overflow-x-auto space-x-3 py-2 px-1 scrollbar-thumb-gray-600 scrollbar-track-gray-900 scrollbar-thin">
              <div 
                ...
                :class="{'border-blue-500': selectedScreenshot && selectedScreenshot.id === screenshot.id, 'border-transparent': !selectedScreenshot || selectedScreenshot.id !== screenshot.id}"
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2 space-y-8">
              ...
              <section>
                ...
                <div v-html="game.description" class="text-white leading-relaxed"></div>
              </section>
              
              <!-- Reviews preview section -->
              <section class="bg-gray-800 rounded-lg p-6 hover:bg-gray-750 transition-colors duration-200">
                ...
                <template v-else-if="reviews.length === 0">
                  <p class="text-gray-400 py-2">No reviews available for this game.</p>
                </template>
                
                <template v-else>
                  <!-- Show just 1 review as a preview -->
                  <div 
                  ...
                  >
                    <div class="flex justify-between items-start mb-3">
                      <div class="flex items-center gap-3">
                        <img 
                          ...
                          @error="event => event.target.src = '/images/default-avatar.jpg'"
                        />
                        <div v-else class="w-10 h-10 rounded-full bg-gray-600 flex items-center justify-center text-xl font-bold">
                          {{ (reviews[0].user?.username || 'U')[0] }}
                        </div>
                        
                        <div>
                          <div class="font-semibold">{{ reviews[0].user?.username || 'Anonymous' }}</div>
                          <div class="text-gray-400 text-sm">{{ formatReviewDate(reviews[0].created) }}</div>
                        </div>
                      </div>
                      
                      <div 
                        v-if="reviews[0].rating" 
                        :class="['px-2 py-1 rounded text-xs font-bold', reviewRatingColor(reviews[0].rating)]"
                      >
                        {{ reviews[0].rating }}
                      </div>
                    </div>
                    ...
                    <div class="text-gray-300">
                      <h4 v-if="reviews[0].title" class="font-bold text-white mb-1">{{ reviews[0].title }}</h4>
            ...
            <div class="space-y-8">
                <!-- Trailers section -->
          <aside v-if="trailers.length > 0" class="bg-gray-800 rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
              <h3 class="text-xl font-bold">Trailers</h3>
              <button 
            ...
            <div class="grid grid-cols-1 gap-2">
              ...
              <aside class="bg-gray-800 rounded-lg p-6">
                ...
                <div class="space-y-4">
                  ...
                  <div v-if="game.platforms && game.platforms.length">
                    <h4 class="text-gray-400 mb-1">Platforms</h4>
                  ...
                  <div v-if="game.genres && game.genres.length">
                    <h4 class="text-gray-400 mb-1">Genres</h4>
                    <div class="flex flex-wrap gap-2">
                      <span 
                  ...
                  <div v-if="game.developers && game.developers.length">
                    <h4 class="text-gray-400 mb-1">Developers</h4>
                    <div class="flex flex-wrap gap-2">
                      <span 
                  ...
                  <div v-if="game.publishers && game.publishers.length">
                    <h4 class="text-gray-400 mb-1">Publishers</h4>
                    <div class="flex flex-wrap gap-2">
                      <span 
                  ...
                  <div v-if="game.rating">
                    ...
                    <div class="flex items-center">
                      ...
                      <span class="text-gray-400 ml-2">({{ game.ratings_count }} ratings)</span>
                    </div>
                  </div>
                  
                  <!-- Website -->
                  <div v-if="game.website">
                    <h4 class="text-gray-400 mb-1">Website</h4>
                    <a 
                      :href="game.website" 
                      target="_blank" 
              ...
              <!-- Tags -->
              <aside v-if="game.tags && game.tags.length" class="bg-gray-800 rounded-lg p-6">
        ...
        >
          ...
          >
            <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-700">
              <h3 class="text-lg sm:text-2xl font-bold text-white">Trailers for {{ game.name }}</h3>
              <button 
...
></iframe>
                ...
                <h4 class="mt-3 text-base sm:text-lg font-semibold text-white">{{ selectedTrailer.name }}</h4>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                <div 
                  ...
                  :class="{
                    'border-blue-500': selectedTrailer && selectedTrailer.id === trailer.id,
                    'border-transparent': !selectedTrailer || selectedTrailer.id !== trailer.id
                  }"
                  @click="selectTrailer(trailer)"
                >
                <img 
                :src="trailer.preview || `https://img.youtube.com/vi/${trailer.id}/maxresdefault.jpg`" 
                :alt="trailer.name" 
                class="w-full h-24 object-cover"
              />
                  <div class="p-2 bg-gray-800 text-sm">
                    {{ trailer.name }}
                  </div>
                </div>
              </div>
          ...
          <div class="max-w-screen-lg max-h-screen p-4" @click.stop>
            ...
            >
              <span class="block w-6 h-6 relative">
                <span class="block absolute w-full h-0.5 bg-white top-1/2 left-0 -rotate-45"></span>
                <span class="block absolute w-full h-0.5 bg-white top-1/2 left-0 rotate-45"></span>
              </span>
            </button>
          </div>
        ...
        >
          ...
          >
            <div class="flex items-center justify-between p-6 border-b border-gray-700">
              <h3 class="text-2xl font-bold text-white">Reviews for {{ game.name }}</h3>
              <button 
                class="text-gray-400 hover:text-white"
                @click="closeReviewsModal"
              >
                <span class="block w-6 h-6 relative">
                  <span class="block absolute w-full h-0.5 bg-current top-1/2 left-0 -rotate-45"></span>
                  <span class="block absolute w-full h-0.5 bg-current top-1/2 left-0 rotate-45"></span>
                </span>
              </button>
            </div>
                ...
                <div class="space-y-6">
                  ...
                  >
                    <div class="flex justify-between items-start mb-4">
                      <div class="flex items-center gap-3">
                        <img 
                          ...
                          @error="event => event.target.src = '/images/default-avatar.jpg'"
                        />
                        <div v-else class="w-10 h-10 rounded-full bg-gray-600 text-wjite flex items-center justify-center text-xl font-bold">
                          {{ (review.user?.username || 'U')[0] }}
                        </div>
                        
                        <div>
                          <div class="font-semibold text-white">{{ review.user?.username || 'Anonymous' }}</div>
                          <div class="text-gray-400 text-sm">{{ formatReviewDate(review.created) }}</div>
                        </div>
                      </div>
                      
                      <div class="flex gap-1">
                        <div 
                          :class="['px-2 py-1 rounded text-xs font-bold', reviewRatingColor(review.rating)]"
                        >
                    ...
                    <div class="text-gray-300 mb-3">
                      <h4 v-if="review.title" class="font-bold text-white mb-2">{{ review.title }}</h4>
                      <div class="text-sm leading-relaxed whitespace-pre-line">{{ review.text }}</div>
                    </div>
            ...
            <div class="border-t border-gray-700 p-4 flex justify-center items-center gap-2">
              <button 
                v-if="reviewPage > 1" 
                @click="reviewPage--" 
              ...
              <span class="px-4 py-2 bg-gray-800 rounded">Page {{ reviewPage }}</span>
              <button 
                v-if="hasMoreReviews" 
                @click="reviewPage++" 
                class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded transition"
  ...
  const { fetchGameDetails, fetchGameScreenshots, fetchGameReviews, searchYouTubeTrailers } = useRawgApi()
